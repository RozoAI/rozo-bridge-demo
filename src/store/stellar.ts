...
interface StellarWalletStore extends StellarWalletState {
  ...
  // Memo validation
  validateMemoRequired: (destinationAddress: string) => Promise<MemoRequiredResponse>
...
export const useStellarWallet = create<StellarWalletStore>()(
  persist(
    (set, get) => ({
      ...
      validateMemoRequired: async (destinationAddress: string): Promise<MemoRequiredResponse> => {
        const { network } = get()
...
export const useStellarMemoValidation = () => {
  const validateMemoRequired = useStellarWallet(state => state.validateMemoRequired)
