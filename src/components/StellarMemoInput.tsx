...
export function StellarMemoInput({
        ...
        if (result.memo_required && result.memo_type) {
          ...
          const defaultMemoType = (() => {
            switch (result.memo_type) {
              case 'text': return MemoType.MemoText
              case 'id': return MemoType.MemoId
              case 'hash': return MemoType.MemoHash
              default: return MemoType.MemoText
            }
          })()
  ...
  const getMemoTypeLabel = (type: MemoType): string => {
    switch (type) {
      case MemoType.MemoText: return 'Text'
      case MemoType.MemoId: return 'ID'
      case MemoType.MemoHash: return 'Hash'
      case MemoType.MemoReturn: return 'Return'
      default: return 'Text'
    }
  }
  ...
  const getMemoPlaceholder = (type: MemoType): string => {
    switch (type) {
      case MemoType.MemoText: return 'Enter memo text (max 28 characters)'
      case MemoType.MemoId: return 'Enter memo ID (number)'
      case MemoType.MemoHash: return 'Enter memo hash (32 bytes hex)'
      case MemoType.MemoReturn: return 'Enter return hash (32 bytes hex)'
      default: return 'Enter memo'
    }
  }
  ...
  const validateMemoValue = (type: MemoType, value: string): string | null => {
    ...
    switch (type) {
      ...
      case MemoType.MemoId:
        ...
        if (BigInt(value) > BigInt('18446744073709551615')) return 'ID memo too large'
        break
      case MemoType.MemoHash:
      case MemoType.MemoReturn:
        if (!/^[0-9a-fA-F]{64}$/.test(value)) return 'Hash memo must be 64 hex characters'
        break
    }
    return null
  }
    ...
    <div className={cn('space-y-4', className)}>
      ...
      {memoRequired && (
        <div className="p-3 bg-yellow-50 dark:bg-yellow-950/20 rounded-lg border border-yellow-200 dark:border-yellow-800">
          <div className="flex items-center gap-2 text-yellow-800 dark:text-yellow-400 mb-2">
            ...
            <Badge variant="secondary" className="text-xs">SEP-29</Badge>
          </div>
          <p className="text-sm text-yellow-700 dark:text-yellow-500">
            This destination address requires a memo. Transactions without a memo may be lost.
          </p>
          {isMuxedAddress(destinationAddress) && (
            <p className="text-xs text-yellow-600 dark:text-yellow-400 mt-1">
              Note: This is a muxed address (M...) which may have built-in routing.
            </p>
          )}
        </div>
      )}
      ...
      <div className="space-y-3">
        ...
        <div className="flex gap-2">
          <Select value={memoType.toString()} onValueChange={handleMemoTypeChange}>
            ...
            <SelectContent>
              ...
              <SelectItem value={MemoType.MemoReturn.toString()}>Return</SelectItem>
            </SelectContent>
          </Select>
        ...
        <div className="text-xs text-muted-foreground">
          <div className="flex items-center gap-1">
            ...
            <span>
              ...
              {(memoType === MemoType.MemoHash || memoType === MemoType.MemoReturn) && ' (32-byte hex)'}
            </span>
          </div>
        </div>
        ...
        {memoValue && (
          <div className="text-xs text-muted-foreground text-right">
            {memoType === MemoType.MemoText && `${memoValue.length}/28 characters`}
            {memoType === MemoType.MemoId && `Value: ${memoValue}`}
            {(memoType === MemoType.MemoHash || memoType === MemoType.MemoReturn) && 
